...
# === Función para descargar HTML (opcional en análisis) ===
...
# === Función para evaluar el certificado ===
def evaluar_certificado(cert, dominio):
    ...
    phishing_keywords = ["login", "secure", "verify", "account", "wallet", "crypto", "exchange", "update", "signin", "bank", "alert", "security"]
    ...
    if any(org.lower() in cert.get("organizationName", "").lower() for org in trusted_orgs): score -= 1; breakdown.append("✅ Organización reconocida (-1)")
    ...
    if cert.get("validDays", 0) <= 90: score += 2; breakdown.append("❌ Certificado de corta duración (<= 90 días) (+2)")
    ...
    if domain_len > 30: score += 1; breakdown.append("❌ Dominio largo (>30) (+1)")
    ...
    if any(alg in cert.get("signatureAlgorithm", "").lower() for alg in weak_algorithms): score += 2; breakdown.append("❌ Algoritmo débil (+2)")
    if re.search(r"([bcdfghjklmnpqrstvwxyz]{4,}){2,}", dominio.replace(".", "")): score += 1; breakdown.append("❌ Dominio aleatorio sospechoso (+1)")
    ...
    if html:
        ...
        if "bitcoin" in soup.get_text().lower():
            score += 2; breakdown.append("❌ Contenido sospechoso detectado: 'bitcoin' (+2)")
    ...
    for real in bancos_reales:
        ...
        if ratio >= 0.85 and full_domain != real:
            score += 2; breakdown.append(f"❌ Muy similar a {real} (Levenshtein +2)")
            break
...
# === Rutas Flask ===
@app.route('/certificado/<host>', methods=['GET'])
def certificado_raw(host):
    try:
